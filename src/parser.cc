  ...
  namespace util {
    namespace parser {
      ...
      void Parser::parseElement (const XMLElement* element, ObjectFactory* parent)
      {
        ...
        for (const XMLNode* el = element->FirstChild ();
      ...
      ObjectFactory* Parser::root () const
      {
        return root_;
      }
      ...
      {
        if (element_ != NULL) tagName_ = element_->ValueStr ();
        else                  tagName_ = "unamed_tag";
        if (parent_ == NULL) {
          root_ = this;
        }
        else {
      ...
      ObjectFactory::ObjectFactory (const std::string& tagName,
          ObjectFactory* parent)
        : parent_ (parent), root_ (NULL), element_ (NULL),
        tagName_ (tagName), id_ (-1)
      {
        if (parent_ == NULL) root_ = this;
        else {
          root_ = parent_->root ();
          parent_->addChild (this);
        }
      ...
      XMLNode* ObjectFactory::write (XMLNode* parent) const
      {
        ...
        for (AttributeMap::const_iterator it = attrMap_.begin ();
            it != attrMap_.end (); ++it)
